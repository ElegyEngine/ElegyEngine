...
{
	private static ElegyPlatformGraphics? mGraphics;
	...
	public static ElegyPlatformGraphics Graphics
		=> mGraphics ?? throw new InvalidOperationException( $"{nameof( ElegyPlatform )} hasn't been initialized" );

	public static void Initialize( bool passthrough = false )
	{
		AvaloniaSynchronizationContext.AutoInstall = false; // Elegy has its own sync context, don't replace it

		mGraphics = new ElegyPlatformGraphics();
		...
		if ( !passthrough )
		{
			AvaloniaLocator.CurrentMutable
				...
				.Bind<IPlatformGraphics>().ToConstant( mGraphics )
		...
		mCompositor = new AvCompositor( mGraphics );
	}
