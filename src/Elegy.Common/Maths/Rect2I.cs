...
// https://github.com/godotengine/godot/blob/master/modules/mono/glue/GodotSharp/GodotSharp/Core/Rect2I.cs
...
namespace Elegy.Common.Maths
{
	...
	public struct Rect2I : IEquatable<Rect2I>
	{
		...
		{
			Vector2I end = End;
			Vector2I topLeft = new Vector2I( Math.Min( _position.X, end.X ), Math.Min( _position.Y, end.Y ) );
		...
		public readonly Rect2I Intersection( Rect2I b )
		{
			...
			newRect._position.X = Math.Max( b._position.X, _position.X );
			newRect._position.Y = Math.Max( b._position.Y, _position.Y );

			Vector2I bEnd = b._position + b._size;
			Vector2I end = _position + _size;

			newRect._size.X = Math.Min( bEnd.X, end.X ) - newRect._position.X;
			newRect._size.Y = Math.Min( bEnd.Y, end.Y ) - newRect._position.Y;
		...
		public readonly Rect2I Merge( Rect2I b )
		{
			...
			newRect._position.X = Math.Min( b._position.X, _position.X );
			newRect._position.Y = Math.Min( b._position.Y, _position.Y );

			newRect._size.X = Math.Max( b._position.X + b._size.X, _position.X + _size.X );
			newRect._size.Y = Math.Max( b._position.Y + b._size.Y, _position.Y + _size.Y );
